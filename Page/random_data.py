...
def get_mobile():
    ...
    b = str(a)
    time.sleep(1)
    return random.choice(prelist) + b[2:]
...
def get_org_no():
    return str(random.randint(10000000, 99999999)) + '-' + str(random.randint(1, 10))  # 随机组织机构代码


def getCompanyName():
    # 获取公司名称
    _last = last_name(1, last_names)
    return "中国{}{}有限公司".format(_last, first_name(3, last_names, first_names))


def getContractName():
    _last = last_name(1, last_names)
    return "{}{}".format(_last, first_name(3, last_names, first_names))
...
def random_str(randomlength=4):
    ...
    random.shuffle(a)
    return str.upper(''.join(a[:randomlength]))


def setting_input_time(driver):
    js = '''var rs = document.querySelectorAll("input[readonly='readonly']");
...
def getDistrictCode():
    ...
    Id += str(random.randint(1998, 2013))  # 年份项
