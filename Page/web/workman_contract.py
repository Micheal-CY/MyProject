...
from Page.Element import get_element, get_elements, is_element_present, get_element_3s, is_element_present_3s
...
def workman_contract(driver, people, out_of_date):
    try:
        ...
        get_element(driver, ('xpath', "//input[@placeholder='请输入姓名查询']")).send_keys(people)
        sleep(0.5)
        workman_element = "//span[contains(text(),'%s')]" % people
        get_element(driver, ('xpath', "//input[@placeholder='请输入姓名查询']")).send_keys(Keys.ENTER)
        sleep(1)
...
def edit_workers_contract(driver, workers_name, photo_path):
    try:
        ...
        get_element(driver, ('xpath', "//input[@placeholder='请输入加班单价']")).clear()
        sleep(1)
        get_element(driver, ('xpath', "//input[@placeholder='请输入加班单价']")).send_keys(new_overtime_work)
        ...
        workers_contract_time = "//div[@id='laborContract']/p[contains(text(),'1.用工日期从%s进场始至%s日止')]" % (str(get_pass_dates(2)), str(get_future_date(150)))
...
def termination_of_the_contract(driver, workers_name):
    try:
        ...
        get_element(driver, ('xpath', "//div[@id='table-toolbar']/button")).click()
        sleep(1)
        if is_element_present_3s(driver, ('xpath', "//a[contains(text(),'合同详情') and @class='table-a-link']"), 2):
